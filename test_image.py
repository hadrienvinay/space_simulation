...
def main():
    ...
    while running:
        if menu == 0:   
            print(menu)
            text_surface = font.render("MENU - SSS", False, (255, 215, 255, 1))
            text_width, text_height = text_surface.get_size()
            text_data = pygame.image.tostring(text_surface, "RGBA", True)
            button_earth = Button (pygame.Rect(WIDTH-180, 100, 150, 50),
                                font.render("No Speed", False, (0, 0, 0, 255)),
                                WHITE_3D,
                                font)
            button_solar_system = Button (pygame.Rect(WIDTH-180, 20, 150, 50),
            ...
            while run:
                ...
                if menu == 1 :
                    ...
                    gluLookAt(camera.pos_x, camera.pos_y,camera.pos_z, camera.angle_x, camera.angle_y,camera.angle_z, 0, 0, 1)
